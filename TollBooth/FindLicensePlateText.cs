...
public class FindLicensePlateText
{
    ...
    private async Task<string> MakeOCRRequest( byte[] imageBytes )
    {
        ...
        this._client.DefaultRequestHeaders.Add( "Ocp-Apim-Subscription-Key", apiKey );
        ...
        {
            this._log.LogCritical( $"Could not contact the Computer Vision API service due to the following error: {bce.Message}" );
        }
        catch( Exception e )
        {
            this._log.LogCritical( $"Critical error: {e.Message}", e );
        }

        this._log.LogInformation( $"Finished OCR request. Result: {licensePlate}" );
    ...
    {
        if( ( result.Regions is null ) || ( result.Regions.Length == 0 ) )
        {
            return string.Empty;
        }
        ...
        string[] stateList = STATES.Split( new[] { ',' }, StringSplitOptions.RemoveEmptyEntries );

        // We are only interested in the first region found, and only the first two lines within the region.
        string text = string.Empty;
        foreach( Line line in result.Regions[0].Lines.Take( 2 ) )
        {
            // Exclude the state name.
            if( stateList.Contains( line.Words[0].Text.ToUpper( CultureInfo.CurrentCulture ) ) )
            {
                continue;
            }
            ...
            {
                if( string.IsNullOrWhiteSpace( word.Text ) == false )
                {
        ...
        return text.ToUpper( CultureInfo.CurrentCulture ).Trim();
    }
    ...
    private AsyncPolicyWrap<HttpResponseMessage> DefineAndRetrieveResiliencyStrategy()
    {
        ...
        Polly.Retry.AsyncRetryPolicy<HttpResponseMessage> waitAndRetryPolicy = Policy
            ...
            .OrResult<HttpResponseMessage>( e => e.StatusCode is HttpStatusCode.ServiceUnavailable or
                ( (HttpStatusCode) 429 ) or ( (HttpStatusCode) 403 ) )
            .WaitAndRetryAsync( 10, // Retry 10 times with a delay between retries before ultimately giving up
                ...
                {
                    this._log.LogWarning( $"Computer Vision API server is throttling our requests. Automatically delaying for {calculatedWaitDuration.TotalMilliseconds}ms" );
                }
            );
        ...
        AsyncCircuitBreakerPolicy<HttpResponseMessage> circuitBreakerPolicyForRecoverable = Policy
            ...
            .CircuitBreakerAsync(
                ...
                {
                    this._log.LogWarning( $"Polly Circuit Breaker logging: Breaking the circuit for {breakDelay.TotalMilliseconds}ms due to: {outcome.Exception?.Message ?? outcome.Result.StatusCode.ToString()}" );
                },
                onReset: () => this._log.LogInformation( "Polly Circuit Breaker logging: Call ok... closed the circuit again" ),
                onHalfOpen: () => this._log.LogInformation( "Polly Circuit Breaker logging: Half-open: Next call is a trial" )
            );
